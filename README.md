...
- **细化的结构解析**：抽出章节树（含行号范围与层级路径）、表格（行列与对齐）、列表（层级与任务勾选）、代码块（语言、是否闭合）、引用块、图片、链接、脚注、行内代码、setext 标题。AI 可按章节逐节精读。
- **结构信息块**：输出里默认附带一份由脚本自动生成的「文档结构信息」（章节分布、代码块语言、表格行列、任务完成度、篇幅等），开箱可见。
...
5. **文档结构信息**：默认附带（`include_structure`），脚本自动生成的章节/代码块/表格/列表/链接统计，可折叠。

可选：开启 `include_section_points` 且 `summaries.json` 里写了 `sections` 时，追加「章节要点」块。
